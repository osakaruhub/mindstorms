...
public class HTEopdShortSensor extends Sensor
{
   ...
   * The short distance mode is used to detect white objects at close distance.
  ...
  protected void init()
  {
    if (LegoRobot.getDebugLevel() >= SharedConstants.DEBUG_LEVEL_MEDIUM)
      DebugConsole.show("htes.init()");
  }

  protected void cleanup()
  {
    if (LegoRobot.getDebugLevel() >= SharedConstants.DEBUG_LEVEL_MEDIUM)
      DebugConsole.show("htes.cleanup()");
    pd.close();
  }
  ...
  {
    if (robot == null)
      new ShowError("HTEopdShort (port: " + getPortLabel()
